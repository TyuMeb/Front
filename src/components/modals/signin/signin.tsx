      ...
      .then(({ access, refresh }) => {
        ...
        getUser()
          ...
          .then((user) => {
            ...
            dispatch(closeModal());
            user.role === "contractor" ? router.push("/account/my-offer") : router.push("/account/customer-orders");
          });
      })
      .catch((error) => {
  ...
  return (
    <ModalAuth>
      ...
      <form className={cx("form")} onSubmit={handleSubmit(onSubmit)}>
        ...
        <button className={cx("linkText")} type="button" onClick={() => dispatch(setTypeModal("resetPassword"))}>
          Забыли пароль?
        </button>

        {isSuccessVerify && <p className={styles.textSuccess}>Ваш аккаунт подтвержден</p>}
        {errorVerify && <p className={cx("textError", { warningText: true })}>Ссылка для подтверждения неактивна</p>}
